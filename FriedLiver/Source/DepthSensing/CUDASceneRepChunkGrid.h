...
namespace std {
template<>
struct hash<SDFBlockDesc> : public std::unary_function<SDFBlockDesc, size_t> {
...
public:
			...
			if (m_grid[i] != NULL)	{
				...
				for (unsigned int k = 0; k < descs.size(); k++) {
					...
					for (unsigned int l = 0; l < linearBlockSize; l++) {
						if (blocks[k].data[l].weight > 0 && fabsf(blocks[k].data[l].sdf) <= thresh) {// std::fabsf-->fabsf change by guan
	...
	void startMultiThreading() {
		...
		//hStreamingThread = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)StreamingFunc, (LPVOID)this, 0, &dwStreamingThreadID); chang by guan
	...
	void stopMultiThreading() {
		...
		if (!s_terminateThread) {
			//WaitForSingleObject(hEventOutProduce, INFINITE);// change by guan
	...
	void deleteCritialSection() {
		//CloseHandle(hMutexOut);//change by guan
	...
	//	hEventOutProduce = CreateEvent(NULL, FALSE, TRUE, NULL);//change by guan
