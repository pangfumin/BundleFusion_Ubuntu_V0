...
///////////////////////////////////////////////////////////////////////////////////
//#include <cstddef>//here is confusing
namespace stb {// here has some question########################
	//#define STB_IMAGE_IMPLEMENTATION//change by guan
...
//#define STB_IMAGE_WRITE_IMPLEMENTATION //add by guan
//#include "sensorData/stb_image_write.h"

//#define STB_IMAGE_IMPLEMENTATION  //add by guan
//#include "sensorData/stb_image.h"
			...
			vec3uc* decompressColorAlloc_stb(COMPRESSION_TYPE_COLOR type) const {	//can handle PNG, JPEG etc.
				...
				int width, height;
                               ...
                               // int w, h, n;
                                //rgba
                                //load image
                               // unsigned char *data = stb::stbi_load("rgba.png", &w, &h, &n, 0);
...
#define M_SENSOR_DATA_VERSION 4
		...
		public:
			...
			std::string getCurrent() {
				std::stringstream ss;
				ss << m_base;
				for (unsigned int i = std::max(1u, (unsigned int)std::ceil(std::log10((float)m_current + 1))); i < m_numCountDigits; i++) ss << "0";//std::ceilf change to std::ceil std::log10f change to std::log10
				ss << m_current;
				ss << m_fileEnding;
				return ss.str();
			}
